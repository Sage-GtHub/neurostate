import { Button } from "@/components/ui/button";
import { ArrowRight } from "lucide-react";

const Hero = () => {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-carbon">
      {/* Video Background */}
      <div className="absolute inset-0">
        <video
          autoPlay
          loop
          muted
          playsInline
          className="w-full h-full object-cover opacity-40"
        >
          <source src="/neural-network-bg.mp4" type="video/mp4" />
        </video>
        <div className="absolute inset-0 bg-gradient-to-b from-carbon/60 via-carbon/40 to-carbon/80" />
      </div>

      {/* Content */}
      <div className="relative z-10 container mx-auto px-6 md:px-12 lg:px-20 xl:px-32 py-32">
        <div className="max-w-6xl mx-auto text-center space-y-10">
          <div className="inline-block px-6 py-3 bg-white/10 border border-white/20 mb-6 backdrop-blur-sm">
            <p className="text-white text-xs tracking-[0.3em] uppercase font-medium">
              Neuromodulation Technology
            </p>
          </div>

          <h1 className="text-6xl md:text-8xl lg:text-9xl font-light text-white leading-[1.05] tracking-tight">
            Engineer your
            <br />
            <span className="font-normal">
              cognitive peak
            </span>
          </h1>

          <p className="text-xl md:text-2xl text-white/80 max-w-3xl mx-auto font-light leading-relaxed">
            AI-powered performance optimisation, photobiomodulation devices, 
            and precision supplements for the modern human.
          </p>

          <div className="flex flex-col sm:flex-row gap-4 justify-center pt-10">
            <Button 
              size="lg"
              className="bg-white text-carbon hover:bg-white/90 text-base px-10 py-7 rounded-full group transition-all duration-300"
            >
              Explore the System
              <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </Button>
            <Button 
              variant="outline"
              size="lg"
              className="border-white/40 text-white hover:bg-white/10 text-base px-10 py-7 rounded-full transition-all duration-300 backdrop-blur-sm"
            >
              Watch Demo
            </Button>
          </div>

          {/* Scroll Indicator */}
          <div className="absolute bottom-12 left-1/2 -translate-x-1/2 animate-bounce">
            <div className="w-6 h-10 rounded-full border-2 border-white/40 flex items-start justify-center p-2 backdrop-blur-sm">
              <div className="w-1.5 h-3 bg-white/60 rounded-full" />
            </div>
          </div>
        </div>
      </div>

      {/* Gradient Overlay */}
      <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-pearl to-transparent" />
    </section>
  );
};

export default Hero;
